...
{
    encode_ = pdji_motor_instance_->rx_buff[0] << 8 | pdji_motor_instance_->rx_buff[1];
    spd_ = pdji_motor_instance_->rx_buff[2] << 8 | pdji_motor_instance_->rx_buff[3];
    tor_ = pdji_motor_instance_->rx_buff[4] << 8 | pdji_motor_instance_->rx_buff[5];
    temp_ = pdji_motor_instance_->rx_buff[6];
...
void DjiMotor::Init(uint32_t _idx, CAN_HandleTypeDef *_phcan, uint8_t _init)
{
    ...
    conf.hcan = _phcan;
    conf.rx_id = _idx;
    pdji_motor_instance_ = pCanRegister(&conf);
}
